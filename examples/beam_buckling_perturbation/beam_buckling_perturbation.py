...
A static perturbation step is inserted before to apply bending.
...
def main():
    with ccx_model.Model(CCX_PATH, CGX_PATH, jobname='beam_buckling_perturbation', working_dir=WKD) as model:
        ...
        step.add_step_keywords(
            sk.Static(), # Set up a static step
            sk.Cload(pilot, 2, 2000),  # force in Y at pilot node with magnitude 2000
        ...
        # step
        step = sk.Step(perturbation=True) 
        model.add_steps(step)       # add step to model
        # add keywords to the step
        step.add_step_keywords(
